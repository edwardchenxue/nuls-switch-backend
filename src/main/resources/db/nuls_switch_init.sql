...
CREATE TABLE `tx_order` (
  ...
  `tx_type` tinyint(4) DEFAULT NULL COMMENT '交易类型：1-买入、2-卖出',
  `from_token_id` int(11) DEFAULT NULL COMMENT '原token',
  `to_token_id` int(11) DEFAULT NULL COMMENT '目标token',
  `price` decimal(20,8) DEFAULT NULL COMMENT '价格',
  `total_num` int(11) DEFAULT NULL COMMENT '挂单总数量',
  `tx_num` int(11) DEFAULT NULL COMMENT '已完成交易数量',
  `total_amount` decimal(20,8) DEFAULT NULL COMMENT '挂单总金额',
  `status` tinyint(4) DEFAULT NULL COMMENT '状态：0-未交易、1-部分交易、2-完成交易、9-撤销',
...
CREATE TABLE `tx_token_pair` (
  ...
  `from_token_id` int(11) DEFAULT NULL COMMENT '原token',
  `to_token_id` int(11) DEFAULT NULL COMMENT '转换为目标token',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`pair_id`)
...
CREATE TABLE `tx_trade` (
  ...
  `order_id` int(11) DEFAULT NULL COMMENT '委托挂单ID',
  `address` varchar(255) DEFAULT NULL COMMENT '交易用户地址',
  `tx_num` int(11) DEFAULT NULL COMMENT '交易数量',
  `status` varchar(255) DEFAULT NULL COMMENT '状态：0-未交易、1-完成交易、9-撤销',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`tx_id`)
